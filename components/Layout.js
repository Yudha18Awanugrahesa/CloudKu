...
import {
  LayoutDashboard, ArrowLeftRight, Wallet, Target, PiggyBank, FileBarChart2,
  Settings as SettingsIcon, LogOut, Cloud,
} from 'lucide-react';
...
export default function Layout({ children }) {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      <aside className="hidden md:flex w-60 flex-shrink-0 bg-primary-dark flex-col min-h-screen">
        <div className="flex items-center gap-2 px-5 py-5">
          <div className="w-7 h-7 rounded-lg bg-white/15 flex items-center justify-center"><Cloud size={16} color="#fff" /></div>
        ...
        <div className="p-3.5 border-t border-white/10 flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-full bg-white/15 overflow-hidden flex items-center justify-center flex-shrink-0">
            {profile?.avatar_url
              ...
              : <span className="text-white font-bold text-xs">{displayName?.[0]?.toUpperCase()}</span>}
          </div>
          <div className="flex-1 min-w-0 text-white text-xs font-bold truncate">{displayName}</div>
          <button onClick={() => signOut()} className="text-white/70 hover:text-white"><LogOut size={16} /></button>
        </div>
      </aside>

      <main className="flex-1 min-w-0">
        <div className="px-5 pt-5 pb-24 max-w-5xl mx-auto">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex z-40 overflow-x-auto">
        {NAV.map(n => {
          const Icon = n.icon;
          const active = router.pathname === n.href;
          return (

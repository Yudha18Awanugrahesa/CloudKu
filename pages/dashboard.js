...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (!user) return;
    (async () => {
      const [{ data: tx }, { data: gl }, { data: pf }] = await Promise.all([
        supabase
          .from("transactions")
          .select("*")
          .eq("user_id", user.id)
  ...
  const saldo = transactions.reduce(
    (s, t) =>
      s + (t.type === "pemasukan" ? Number(t.nominal) : -Number(t.nominal)),
    0,
  );
  ...
  const byCategory = useMemo(() => {
    ...
    monthTx
      .filter((t) => t.type === "pengeluaran")
      .forEach((t) => {
        map[t.category] = (map[t.category] || 0) + Number(t.nominal);
      });
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            ...
            <SummaryCard
              ...
              value={formatIDR(incomeMonth)}
              icon={TrendingUp}
              tone="good"
            />
            <SummaryCard
              label="Pengeluaran Bulan Ini"
              value={formatIDR(expenseMonth)}
              icon={TrendingDown}
              tone="bad"
            />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5">
            <SummaryCard
              label="Sisa Cash Flow Bulan Ini"
              value={formatIDR(sisaCashflow)}
              icon={ArrowLeftRight}
              tone={sisaCashflow >= 0 ? "good" : "bad"}
            />
            <SummaryCard
              label="Target Tabungan (total)"
              value={formatIDR(totalTargetGoal)}
              icon={Target}
            />
            <SummaryCard
                ...
                <AreaChart data={last14}>
                  <defs>
                    <linearGradient id="gIn" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="#12523F" stopOpacity={0.4} />
                      <stop offset="100%" stopColor="#12523F" stopOpacity={0} />
                    </linearGradient>
                    <linearGradient id="gOut" x1="0" y1="0" x2="0" y2="1">
                      <stop
                        ...
                        stopOpacity={0.35}
                      />
                      <stop offset="100%" stopColor="#B8433A" stopOpacity={0} />
                    </linearGradient>
                  </defs>
                  <CartesianGrid
                    strokeDasharray="3 3"
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-gray-400">
                      {t.category} · {fmtDateShort(t.date)}
                    </div>
                  </div>
                  <div
